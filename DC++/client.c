				...
				for(i=0;i< str_index;i++)	printf("%s\n",fl[i].hostname );
	   			...
	   			for (i=0;i< str_index;i++)
			    	if(strcmp(hostname,fl[i].hostname)==0){
			    		sfd=create_socket(fl[i].port, fl[i].ip);
			    		found=1;
			    		break;
			    	}
...
struct friendList add_friend(struct friendList newFriend)
{
	
	printf("Enter hostname: ");
	scanf("%s",newFriend.hostname);	
	printf("Enter ip: ");
	scanf("%s",newFriend.ip);
	printf("Enter port: ");
	scanf("%d",&newFriend.port);
	return newFriend;
}

// request file list from frienc
void request_list(int sfd, struct sockaddr_in sock_serv ,int l)
{
	long int n,m;
	char buf[BUFFER];
	char command[] = "request file-list";
	bzero(&buf,BUFFER);
	printf("Requesting file list\n");
    m=sendto(sfd,command,strlen(command),0,(struct sockaddr*)&sock_serv,l);
		if(m==-1){
			perror("send error");
			return;
		}
